    ...
    return (
        <div className="p-6 space-y-6">
            ...
            <div className="flex items-center gap-3">
                ...
                <h1 className="text-xl font-semibold text-gray-900 dark:text-white">{beat.name}</h1>
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${ZONE_BADGE[beat.zone_type] ?? 'bg-gray-100 text-gray-600'}`}>
                    {beat.zone_type === 'inclusion' ? 'Inclusion Zone' : 'Exclusion Zone'}
                </span>
            </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 space-y-3">
                    <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Zone Info</h2>
                    ...
                    {beat.description && (
                        <p className="text-sm text-gray-500 border-t border-gray-100 dark:border-gray-700 pt-3">
                            {beat.description}
                        </p>
                    )}
                ...
                <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
                    ...
                    {beat.beat_assignments && beat.beat_assignments.length > 0 ? (
                        <ul className="divide-y divide-gray-100 dark:divide-gray-700">
                            {beat.beat_assignments.map((ba, i) => (
                                <li
                                    key={i}
                                    className="py-2 flex items-center justify-between cursor-pointer hover:bg-gray-50 dark:hover:bg-gray-750 rounded px-1"
                                    onClick={() => ba.device && navigate(`/devices/${ba.device.id}`)}
                                >
                                    <div>
                                        <p className="text-sm font-medium text-gray-900 dark:text-white">
                                            {ba.device?.name ?? '—'}
                                        </p>
                                        <p className="text-xs text-gray-400">{ba.device?.imei}</p>
                                    </div>
                        ...
                        <p className="text-sm text-gray-400">No devices assigned to this beat.</p>
                    )}
                </div>
            </div>
        </div>
