    ...
    useEffect(() => {
        ...
        import('leaflet').then((L) => {
            ...
            const withCoords = devices.filter(
                (d) => d.last_lat != null && d.last_lon != null,
            );
            ...
            if (withCoords.length > 1) {
                const bounds = L.latLngBounds(
                    withCoords.map((d) => [d.last_lat!, d.last_lon!]),
                );
    ...
    useEffect(() => {
        ...
        import('pusher-js').then(({ default: Pusher }) => {
            ...
            ch.bind(
                ...
                (payload: LocationsBatchPayload) => {
                    setDevices((prev) => {
                        const next = [...prev];

                        for (const pos of payload.positions) {
                            const idx = next.findIndex(
                                (d) => d.id === pos.device_id,
                            );
        ...
        <div className="flex h-full flex-col">
            ...
            <div className="flex shrink-0 items-center justify-between border-b border-gray-200 bg-white px-4 py-3 dark:border-gray-800 dark:bg-gray-900">
                ...
                <span className="text-xs text-gray-500">
                    {active.length} / {devices.length} with location · real-time
                    via Soketi
                </span>
            </div>
