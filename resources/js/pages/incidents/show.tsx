        ...
        <div className="p-6 space-y-6">
                ...
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        <h1 className="text-xl font-semibold text-gray-900 dark:text-white">
                            {incident.device?.name ?? 'Unknown Device'} — {incident.event_type}
                        </h1>
                        <div className="mt-1 flex items-center gap-2">
                            ...
                            <span className="text-xs text-gray-500">
                                Level {incident.level} · {new Date(incident.triggered_at).toLocaleString()}
                            </span>
                        </div>
                    </div>
                </div>
            ...
            <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5">
                        ...
                        <dl className="space-y-2">
                            {[
                                ...
                                ['Location', incident.latitude && incident.longitude
                                    ...
                                    : '—'],
                                ['Reopened', incident.reopen_count > 0 ? `${incident.reopen_count}×` : 'Never'],
                        ...
                        {incident.resolution_notes && (
                            <div className="mt-4 pt-4 border-t border-gray-100 dark:border-gray-700">
                                ...
                                <p className="text-sm text-gray-700 dark:text-gray-300">{incident.resolution_notes}</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                {incident.status !== 'resolved' && (
                    <div className="bg-white dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700 p-5 space-y-4 h-fit">
                        <h2 className="text-sm font-semibold text-gray-900 dark:text-white">Update Status</h2>
                        <textarea
                            value={notes}
                            onChange={(e) => setNotes(e.target.value)}
                            placeholder="Resolution notes (optional)"
                            rows={3}
                            className="w-full text-sm rounded-lg border border-gray-200 dark:border-gray-600 bg-gray-50 dark:bg-gray-700 px-3 py-2 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500"
                        />
